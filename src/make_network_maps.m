...
system(['gunzip -f ' outdir '/Yeo.nii.gz']);
yeo_nii = [outdir '/Yeo.nii'];
...
% Compute mean fmri and find a threshold for in-brain voxels
thresh = spm_antimode(Ymeanfmri(:)) / 2;
Ymask = Ymeanfmri >= thresh;
keeps = Ymask(:);
...
maps = zeros(length(systems),length(Ymeanfmri(:)));
for sy = 1:length(systems)
...
msize = [osize(1:3) length(systems)];
...
for sy = 1:length(systems)
	...
	% Print to PNG
	print(gcf,'-dpng',sprintf('%s/connmap_%s_%03d.png',out_dir,filetag,sy))
	close(pdf_figure)
