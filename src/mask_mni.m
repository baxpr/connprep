...
for f = 1:length(fmri_niis)
	...
	% Resample ICV to fMRI space and load. Copy to out_dir first so we
	% aren't trying to store the resliced image in a nonwriteable part of
	% the container
	flags = struct('mask',true,'mean',false,'interp',0,'which',1, ...
        'wrap',[0 0 0],'prefix','r');
	icv_nii = [spm('dir') '/tpm/mask_ICV.nii'];
	copyfile(icv_nii,fullfile(out_dir,'mask_ICV.nii'));
	icv_nii = fullfile(out_dir,'mask_ICV.nii');
